...
export default async function handler(req,res){
 ...
 try{
  ...
  let obs=await call('/api/cmd/RESET',{game_id:GAME_ID,card_id:cardId});
  const guid=obs.guid,ledger=[],receipts=[];let closure=reclose(ledger),levels=Number(obs.levels_completed||0);
  for(let i=0;i<PATH.length;i++){
   ...
   const ret=await call(`/api/cmd/ACTION${action}`,{game_id:GAME_ID,guid,reasoning:JSON.stringify(reasoning)});

...
import { useFirebase } from '../App'; // Import the useFirebase hook
import { appId, enableGoogleSignIn } from '../firebase-config'; // Import configuration
...
const TokenomicsUI = () => {
  ...
  const { auth, db, facebookProvider } = useFirebase();

  // State for authentication
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [loadingAuth, setLoadingAuth] = useState(true); // State to track authentication loading
  const [userId, setUserId] = useState(null); // Add userId state

  // Set up auth state listener
  useEffect(() => {
    if (!auth || !db) {
      console.warn('Firebase Auth or Firestore not initialized yet');
      return;
    }
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        if (user) {
          ...
          // Fetch connected Instagram account from Firestore
          try {
            const userProfileDocRef = doc(db, `artifacts/${appId}/users/${user.uid}/profile`, 'user_data');
            const userDocSnap = await getDoc(userProfileDocRef);
            if (userDocSnap.exists()) {
              const userData = userDocSnap.data();
              if (userData.connectedInstagramAccount) {
                setConnectedInstagramAccount(userData.connectedInstagramAccount);
              } else {
                setConnectedInstagramAccount(null);
              }
            } else {
              // Create a basic user profile document if it doesn't exist
              await setDoc(userProfileDocRef, { lastLogin: new Date().toISOString() }, { merge: true });
              setConnectedInstagramAccount(null);
            }
          } catch (firestoreError) {
            console.error("Error fetching/setting user profile from Firestore:", firestoreError);
            setConnectedInstagramAccount(null);
          }
        } else {
          // User is signed out
  ...
  const handleGoogleSignIn = async () => {
    if (!auth) {
      console.warn("Firebase Auth not initialized yet for Google sign-in.");
      return;
    }
    ...
    try {
      ...
      // Add scopes if needed (e.g., for accessing Google profile)
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        ...
        // Multiple popup requests were made, only the last one will succeed
  ...
  const [userRaffleWinningsCount, setUserRaffleWinningsCount] = useState(0); // Count of winning tokens user holds
  const [userRaffleWinningsAmount, setUserRaffleWinningsAmount] = useState(0); // Actual dollar amount won

  // New state for 24-hour raffle timer
  const [raffleTimer, setRaffleTimer] = useState(null); // Time remaining in seconds
  const [nextRaffleTimestamp, setNextRaffleTimestamp] = useState(null); // Unix timestamp for next raffle
  ...
  const listTokenForSale = () => {
    ...
    setMarketplaceListings(prev => [...prev, {
      ...
      listingPrice: price,
      sellerId: userId || 'anonymous', // Use Firebase userId or 'anonymous'
  ...
  const buyFromMarketplaceListing = (listingId) => {
    ...
    if (listingIndex !== -1) {
      ...
      if (purchasedListing.sellerId === userId) { // Check against current userId
      ...
      // In a real multi-user app, this would update the seller's balance via Firestore
      ...
      setGlobalStats(prevGlobalStats => {
        ...
        const newEarnedToBoughtRatio = newTotalMarketplaceBought > 0 ? prevGlobalStats.totalEarned / newTotalMarketplaceBought : (prevGlobalStats.totalEarned > 0 ? prevGlobalStats.totalEarned : 0);
        const newEarnedCoinValue = newTotalEarned > 0 ? (prevGlobalStats.totalMarketplaceBought / newTotalEarned) / 100 : 0.01; // Divided by 100 as requested
        return {
          ...prevGlobalStats,
  ...
  const runDailyRaffle = useCallback(() => {
    ...
    setUserBalance(prev => prev + calculatedWinnings); // Add winnings to user's balance
  ...
  // Facebook/Instagram Connection Handlers
  const handleConnectWithFacebook = async () => {
    if (!auth || !facebookProvider) {
      setInstagramConnectMessage('Firebase not initialized. Please try again.');
      return;
    }

    setInstagramConnectMessage('Connecting to Facebook...');

    // Check if user is authenticated
    const currentUser = auth.currentUser;
    if (!currentUser) {
      // If user is not authenticated, sign in with Facebook
      try {
        setInstagramConnectMessage('Signing in with Facebook...');
        const result = await fbSignInWithPopup(auth, facebookProvider);

        // Get the Facebook access token
        const accessToken = FacebookAuthProvider.credentialFromResult(result).accessToken;

        // You can use this accessToken to fetch the Instagram account linked to the Facebook account
        // using the Facebook Graph API
        // For now, we'll just use the Facebook display name
        const displayName = result.user.displayName || 'Facebook User';

        console.log('Facebook access token:', accessToken); // For debugging

        setConnectedInstagramAccount(displayName);
        setInstagramConnectMessage('Connected with Facebook successfully!');
        setShowInstagramConnectModal(false);

        try {
          // Save the connection to Firestore
          const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
          const userProfileDocRef = doc(db, `artifacts/${appId}/users/${result.user.uid}/profile`, 'user_data');
          await setDoc(userProfileDocRef, {
            connectedFacebookAccount: result.user.uid,
            facebookDisplayName: displayName,
            facebookAccessToken: accessToken, // Store the access token for future API calls
            lastUpdated: new Date().toISOString()
          }, { merge: true });

          // Update local state
          setConnectedInstagramAccount(displayName);
          setInstagramConnectMessage('Successfully connected with Facebook!');
          setShowInstagramConnectModal(false);
        } catch (error) {
          console.error('Error saving to Firestore:', error);
          setInstagramConnectMessage('Connected to Facebook but failed to save profile. Please try again.');
        }

      } catch (error) {
        console.error('Error signing in with Facebook:', error);
        setInstagramConnectMessage(`Failed to connect with Facebook: ${error.message}`);
      }
    } else {
      // If user is already authenticated, link their Facebook account
      try {
        setInstagramConnectMessage('Linking Facebook account...');
        ...
        try {
          // Save the linked account to Firestore
          const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
          const userProfileDocRef = doc(db, `artifacts/${appId}/users/${currentUser.uid}/profile`, 'user_data');
          await setDoc(userProfileDocRef, {
            connectedFacebookAccount: currentUser.uid,
            facebookDisplayName: displayName,
            facebookAccessToken: accessToken, // Store the access token for future API calls
            lastUpdated: new Date().toISOString()
          }, { merge: true });
          ...
          // Update local state
          setConnectedInstagramAccount(displayName);
          setInstagramConnectMessage('Successfully linked Facebook account!');
          setShowInstagramConnectModal(false);
        ...
        if (error.code === 'auth/credential-already-in-use') {
          setInstagramConnectMessage('This Facebook account is already linked to another user.');
        } else if (error.code === 'auth/email-already-in-use') {
          setInstagramConnectMessage('This email is already in use with another account.');
        } else if (error.code === 'auth/account-exists-with-different-credential') {
          setInstagramConnectMessage('An account already exists with the same email but different sign-in credentials.');
        } else {
          setInstagramConnectMessage(`Failed to link Facebook account: ${error.message}`);
        }
      }
    }
  };

  // Handle disconnecting Facebook/Instagram account
  const handleDisconnectInstagram = async () => {
    ...
    try {
      // Show loading state
      setInstagramConnectMessage('Disconnecting from Facebook...');

      // Unlink Facebook provider if it exists
      if (currentUser.providerData.some(provider => provider.providerId === 'facebook.com')) {
        try {
          await unlink(currentUser, 'facebook.com');
        } catch (unlinkError) {
          console.warn('Error unlinking Facebook provider:', unlinkError);
          // Continue even if unlink fails, as we still want to clean up the Firestore data
        }
      }

      // Update Firestore to remove the connected Facebook account data
      const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
      const userProfileDocRef = doc(db, `artifacts/${appId}/users/${currentUser.uid}/profile`, 'user_data');

      // Only update the Facebook-related fields, keep other user data
      await updateDoc(userProfileDocRef, {
        connectedFacebookAccount: null,
      ...
      // Update local state
      setConnectedInstagramAccount(null);
      setInstagramConnectMessage('Successfully disconnected from Facebook.');
      ...
      if (error.code === 'auth/requires-recent-login') {
        setInstagramConnectMessage('Please sign in again to disconnect your Facebook account.');
      } else {
        setInstagramConnectMessage('Failed to disconnect from Facebook. Please try again.');
      }
    }
  };
  ...
  const handleListInfluencerToken = (influencerId, quantity, pricePerToken, sourceLink) => {
    ...
    setGlobalStats(prevGlobalStats => {
      ...
      const newEarnedToBoughtRatio = prevGlobalStats.totalMarketplaceBought > 0 ? newTotalEarned / prevGlobalStats.totalMarketplaceBought : (prevGlobalStats.totalEarned > 0 ? newTotalEarned : 0);
  ...
  }, [runDailyRaffle]); // runDailyRaffle is a dependency because it's called inside the effect
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 text-white p-6 font-inter">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-4 mb-8 border border-blue-500/20 text-center">
          {loadingAuth ? (
            ...
            isAuthenticated ? (
              <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                <p className="text-lg text-gray-200">
                  Logged in as: <span className="font-semibold text-blue-300 break-all">{userId}</span>
                </p>
                <button
                  onClick={handleLogout}
                  className="bg-red-600 hover:bg-red-700 py-2 px-4 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2"
                >
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 py-2 px-4 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2"
                >
        ...
        {/* User Balance */}
        <div className="bg-black/30 backdrop-blur-sm rounded-2xl p-4 mb-8 border border-green-500/20 text-center">
          <h2 className="text-xl font-bold flex items-center justify-center gap-2">
            <DollarSign className="text-green-400" />
            Your Balance: <span className="text-green-400 text-3xl">${userBalance.toFixed(2)}</span>
          </h2>
        </div>
          ...
          {marketplaceListings.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-80 overflow-y-auto custom-scrollbar">
              {marketplaceListings.map(listing => {
                ...
                // This now checks if the sellerId is the current Firebase userId
                const isCurrentUserListing = listing.sellerId === userId;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredAndSortedInfluencers.map((influencer) => {
            ...
            // Count how many tokens of this influencer type are listed on the marketplace
            const listedCount = marketplaceListings.filter(listing => listing.influencerId === influencer.id).length;
        ...
        {showInstagramConnectModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-6 max-w-md w-full border border-pink-500/30 shadow-2xl">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-white">Connect with Facebook</h3>
                <button
                  onClick={() => {
                    setShowInstagramConnectModal(false);
              ...
              <div className="mb-4">
                <p className="text-gray-300 mb-6">
                  Connect your Facebook account to link with Instagram and start earning tokens.
                </p>

                {instagramConnectMessage && (
                  <p className={`text-sm mb-4 ${instagramConnectMessage.includes('success') ? 'text-green-400' : 'text-red-400'
                    }`}>
                    {instagramConnectMessage}
                  </p>
                )}
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            {influencer.instagramHandle && (
              ...
              >
                @{influencer.instagramHandle} <Instagram className="h-5 w-5" />
              </a>
            )}
...
const CreateInfluencerModal = ({
  ...
  connectedInstagramAccount,
  influencerToEarnFor,
  setInfluencerToEarnFor,
  onConnectInstagram: propConnectInstagram,
  onDisconnectInstagram: propDisconnectInstagram
}) => {
  // Get Firebase services from context
  const { auth, db, facebookProvider } = useFirebase();
